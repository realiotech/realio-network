...
const baseMsgTransferToken: object = { creator: '', index: '', symbol: '', from: '', to: '', amount: 0 }
...
export const MsgTransferTokenResponse = {
  encode(_: MsgTransferTokenResponse, writer: Writer = Writer.create()): Writer {
    return writer
  },
  ...
  decode(input: Reader | Uint8Array, length?: number): MsgTransferTokenResponse {
    const reader = input instanceof Uint8Array ? new Reader(input) : input
    let end = length === undefined ? reader.len : reader.pos + length
    const message = { ...baseMsgTransferTokenResponse } as MsgTransferTokenResponse
    while (reader.pos < end) {
      const tag = reader.uint32()
      switch (tag >>> 3) {
        default:
...
const baseMsgSendFungibleTokenTransfer: object = { creator: '', port: '', channelID: '', timeoutTimestamp: 0, denom: '', amount: 0, receiver: '' }

export const MsgSendFungibleTokenTransfer = {
  encode(message: MsgSendFungibleTokenTransfer, writer: Writer = Writer.create()): Writer {
    if (message.creator !== '') {
      writer.uint32(10).string(message.creator)
    }
    if (message.port !== '') {
  ...
  decode(input: Reader | Uint8Array, length?: number): MsgSendFungibleTokenTransfer {
    const reader = input instanceof Uint8Array ? new Reader(input) : input
    let end = length === undefined ? reader.len : reader.pos + length
    const message = { ...baseMsgSendFungibleTokenTransfer } as MsgSendFungibleTokenTransfer
    while (reader.pos < end) {
      const tag = reader.uint32()
      switch (tag >>> 3) {
        case 1:
...
const baseMsgSendFungibleTokenTransferResponse: object = {}

export const MsgSendFungibleTokenTransferResponse = {
  encode(_: MsgSendFungibleTokenTransferResponse, writer: Writer = Writer.create()): Writer {
    return writer
  },
  ...
  decode(input: Reader | Uint8Array, length?: number): MsgSendFungibleTokenTransferResponse {
    const reader = input instanceof Uint8Array ? new Reader(input) : input
    let end = length === undefined ? reader.len : reader.pos + length
    const message = { ...baseMsgSendFungibleTokenTransferResponse } as MsgSendFungibleTokenTransferResponse
    while (reader.pos < end) {
      const tag = reader.uint32()
      switch (tag >>> 3) {
        default:
