...
import { Params } from "./module/types/asset/params"
import { Token } from "./module/types/asset/token"
import { TokenAuthorization } from "./module/types/asset/token"


export { AssetPacketData, NoData, Params, Token, TokenAuthorization };
...
export default {
	...
	actions: {
		init({ dispatch, rootGetters }) {
			console.log('Vuex module: realiotech.realionetwork.asset initialized!')
	...
	gas: "200000" }, memo})
				...
				if (e == MissingWalletError) {
					throw new Error('TxClient:MsgTransferToken:Init Could not initialize signing client. Wallet is required.')
				}else{
					throw new Error('TxClient:MsgTransferToken:Send Could not broadcast Tx: '+ e.message)
				}
			}
		},
		...
		async MsgTransferToken({ rootGetters }, { value }) {
			try {
				...
				if (e == MissingWalletError) {
					...
					throw new Error('TxClient:MsgTransferToken:Create Could not create message: ' + e.message)
				}
			}
		},
