/* eslint-disable */
import { VoteOption, WeightedVoteOption, voteOptionFromJSON, voteOptionToJSON } from '../gov/gov'
...
export class MsgClientImpl implements Msg {
  ...
  SubmitProposal(request: MsgSubmitProposal): Promise<MsgSubmitProposalResponse> {
    ...
    const promise = this.rpc.request('realiotech.network.gov.Msg', 'SubmitProposal', data)
    return promise.then((data) => MsgSubmitProposalResponse.decode(new Reader(data)))
  }

  Vote(request: MsgVote): Promise<MsgVoteResponse> {
    const data = MsgVote.encode(request).finish()
    const promise = this.rpc.request('realiotech.network.gov.Msg', 'Vote', data)
    return promise.then((data) => MsgVoteResponse.decode(new Reader(data)))
  }

  VoteWeighted(request: MsgVoteWeighted): Promise<MsgVoteWeightedResponse> {
    const data = MsgVoteWeighted.encode(request).finish()
    const promise = this.rpc.request('realiotech.network.gov.Msg', 'VoteWeighted', data)
    return promise.then((data) => MsgVoteWeightedResponse.decode(new Reader(data)))
  }

  Deposit(request: MsgDeposit): Promise<MsgDepositResponse> {
    const data = MsgDeposit.encode(request).finish()
    const promise = this.rpc.request('realiotech.network.gov.Msg', 'Deposit', data)
