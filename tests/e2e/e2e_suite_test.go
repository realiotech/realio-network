...
import (
	...
	"github.com/realiotech/realio-network/tests/e2e/upgrade"
)

const (
	localRepository       = "realio-network"
	localVersionTag       = "latest"
	defaultChainID        = "realionetworklocal_7777-1"
	defaultManagerNetwork = "realionetworklocal"
...
func (s *IntegrationTestSuite) upgrade() {
	...
	// export node .realio-network-tmp to local build/
	...
	node.SetCmd([]string{"realio-networkd", "start", fmt.Sprintf("--chain-id=%s", s.upgradeParams.ChainID)})
	err = s.upgradeManager.RunNode(node)
