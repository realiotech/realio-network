...
// Manager defines a docker pool instance, used to run and interact with realio
...
func (m *Manager) BuildImage(name, version, dockerFile, contextDir string, args map[string]string) error {
	...
	opts := docker.BuildImageOptions{
		...
		BuildArgs:  buildArgs,
		// name with tag, e.g. realio:v0.8.3
	...
	err = m.pool.Retry(
		func() error {
			...
			if c.State.ExitCode != 0 {
				...
				return fmt.Errorf(
					"can't start realio node, container exit code: %d\n\n[error stream]:\n\n%s\n\n[output stream]:\n\n%s",
					c.State.ExitCode,
					errBuf.String(),
					outBuf.String(),
				)
...
// WaitForHeight query realio node every second until node will reach specified height
...
// Makes system call to current node container environment with realio-networkd cli command to get current block height
func (m *Manager) nodeHeight(ctx context.Context) (int, error) {
	exec, err := m.CreateExec([]string{"realio-networkd", "q", "block"}, m.ContainerID())
	if err != nil {
	...
	if errBuff.String() != "" {
		return 0, fmt.Errorf("realio query error: %s", errBuff.String())
	}
	return h, nil
}
