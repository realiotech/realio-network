...
func (p Precompile) Delegate(
	...
	// Parse arguments
	erc20Token, validatorAddress, amount, err := parseDelegateArgs(args)
	if err != nil {
		return nil, err
	}
...
func parseDelegateArgs(args []interface{}) (common.Address, string, *big.Int, error) {
	fmt.Println("parseDelegateArgs", args)
	if len(args) != 3 {
		return common.Address{}, "", nil, fmt.Errorf("invalid number of arguments for delegate")
	}

	erc20Addr, ok := args[0].(string)
	if !ok {
		return common.Address{}, "", nil, fmt.Errorf("invalid erc20Token address")
	}
	erc20Token := common.HexToAddress(erc20Addr)

	validatorAddress, ok := args[1].(string)
	if !ok {
		return common.Address{}, "", nil, fmt.Errorf("invalid validator address")
	}
	...
	return erc20Token, validatorAddress, amount, nil
}
